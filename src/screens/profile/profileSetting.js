...
const ProfileSetting = () => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      ...
      <ScrollView>
        <View style={{ flexDirection: 'row', marginHorizontal: 22, alignItems: 'center', marginTop: 20 }}>
          <TextFormatted
        ...
        <ButtonRow title={'Select your type'} onPress={() => navigation.navigate('selectYourType')} />
        <ButtonRow title={'Filter location'} onPress={() => navigation.navigate('filterLocation')} />
        <ButtonRow title={'Language'} onPress={() => navigation.navigate('ChangeLanguage')} />
        <ButtonRow title={'Logout'} />
      </ScrollView>
      <ButtonRow title={'Close account'} color={theme.colors.darkGrey} onPress={() => refRBSheet.current.open()} />
    ...
    >
      <TextFormatted
        style={{
          fontSize: 14,
          fontWeight: '600',
          color: color || ThemeMode.selectedTheme ? theme.colors.primaryBlack : theme.colors.primary,
          flex: 1,
        }}
      >
        {title}
      </TextFormatted>
      <Icon name="chevron-small-right" size={20} color={color || ThemeMode.selectedTheme ? theme.colors.primaryBlack : theme.colors.primary} />
    </TouchableOpacity>
  );
};
