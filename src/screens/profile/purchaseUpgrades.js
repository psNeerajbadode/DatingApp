...
const PurchaseUpgrades = ({ refRBSheet, refRBSheet2 }) => {
  ...
  const upgradeData = [
    {
      img: ThemeMode.selectedTheme ? require('../../assets/icons/prop_llik.png') : require('../../assets/icons/like_dark2.png'),
      flowers: [
    ...
    >
      ...
      >
        ...
        <Fontisto
          ...
          size={14}
          color="#fff"
          style={{ alignSelf: 'flex-end', marginRight: 30, marginTop: 0, padding: 10 }}
          onPress={() => {
          ...
          renderItem={({ item, index }) => (
            <View style={{ marginTop: 0 }}>
              <View style={{ justifyContent: 'center', alignItems: 'center', alignSelf: 'center' }}>
                <Image source={item.img} style={{ height: 110, width: 110, resizeMode: 'contain' }} />
              </View>
              ...
              {item.flowers.map((v, i) => (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderWidth: 2,
                    borderColor: '#fff',
                    backgroundColor: select == v.qut ? theme.colors.primary : 'transparent',
                    paddingVertical: 20,
                    borderRadius: 10,
                  }}
                >
                  <TextFormatted
                    style={{ fontSize: 16, fontWeight: '400', color: select == v.qut ? colormode : theme.colors.primary, textAlign: 'center' }}
                  >
                    {v.qut}
                  </TextFormatted>
                  <TextFormatted
                    style={{ fontSize: 30, fontWeight: '700', color: select == v.qut ? colormode : theme.colors.primary, textAlign: 'center' }}
                  >
                    {v.price}
                  </TextFormatted>
                  <View
                    style={{
                      position: 'absolute',
                      backgroundColor: select == v.qut ? v?.offer && colormode : v?.offer && theme.colors.primary,
                      paddingVertical: 2,
                      paddingHorizontal: 6,
                      right: 0,
