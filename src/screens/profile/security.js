import { FlatList, Image, ImageBackground, ScrollView, StatusBar, StyleSheet, Text, TouchableOpacity, useWindowDimensions, View } from 'react-native';
...
const Security = () => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      ...
      <ScrollView>
        <ButtonRow title={'Change password'} onPress={() => refRBSheet.current.open()} />
        <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 20, marginTop: 10, marginRight: 20, marginBottom: 10 }}>
          <TextFormatted
        ...
        <ButtonRow title={'Manage payments account'} onPress={() => refRBSheet1.current.open()} />
          ...
          <ScrollView keyboardShouldPersistTaps="handled">
            <TextInputFormat
              ...
              labelColor={
                ...
                //   ? '#EA4A5A'
                ThemeMode.selectedTheme ? theme.colors.primaryBlack : theme.colors.primary
              }
              borderWidth={oldPassword == '' || (oldPassword != null && !validPass(oldPassword)) ? 1 : 0}
              ...
              showMess={oldPassword == '' || (oldPassword != null && !validPass(oldPassword)) ? true : false}
            ...
            <TextInputFormat
              ...
              containerStyle={{ marginTop: 20 }}
              showMess={newPassword == '' || (newPassword != null && !validPass(newPassword)) ? true : false}
              mess={'Password should be minimum 8 digits length with 1 Uppercase and 1 number'}
            />
            <TextInputFormat
              ...
              borderWidth={repeatPassword == '' || (repeatPassword != null && !validPass(repeatPassword)) ? 1 : 0}
              right={
                <Ionicons
              ...
              containerStyle={{ marginTop: 20 }}
              showMess={repeatPassword == '' || (repeatPassword != null && !validPass(repeatPassword)) ? true : false}
              // mess={'Password should be minimum 8 digits length with 1 Uppercase and 1 number'}
              mess={'password is matching the expectation but its not the same as the above password'}
            />
            <View style={{ height: 20 }} />
          </ScrollView>
          <ButtonView>
            <TextFormatted
              style={{ fontSize: 18, fontWeight: '700', color: '#8490AE', width: dimension.width / 2 - 20, textAlign: 'center', paddingVertical: 5 }}
              onPress={() => {
            ...
            <Button
              opacity={validPass(oldPassword) && validPass(newPassword) && validPass(repeatPassword) ? 1 : 0.5}
              buttonName={'Save'}
              ...
              disabled={validPass(oldPassword) && validPass(newPassword) && validPass(repeatPassword) ? false : true}
            />
          </ButtonView>
        </RBSheet>
        {/* </BottomSheet> */}
...
const ButtonRow = ({ title, onPress }) => {
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 20, paddingVertical: 5, marginVertical: 5 }}
    >
      <TextFormatted
        style={{ fontSize: 14, fontWeight: '600', color: ThemeMode.selectedTheme ? theme.colors.primaryBlack : theme.colors.primary, flex: 1 }}
      >
...
const CreditCard = ({ refRBSheet1, autoMonthlyPayment, setAutoMonthlyPayment, refRBSheet2, refRBSheet3 }) => {
  const ThemeMode = useSelector(state => state.Theme);
  const dimension = useWindowDimensions();
  ...
  return (
    ...
    >
      ...
      <ScrollView>
          ...
          renderItem={({ item, index }) => (
            <View style={{}}>
              ...
              >
                <ImageBackground
                  source={item.img}
                  style={{ height: 200, width: dimension.width - 45, alignSelf: 'center', marginLeft: 20, marginRight: 20, marginTop: 20 }}
                  imageStyle={{ borderRadius: 40 }}
                  resizeMode="cover"
                >
                  <View style={{ height: 110 }} />
                  <View style={{ marginHorizontal: 25 }}>
                    <TextFormatted style={{ fontSize: 21, fontWeight: '700', color: theme.colors.primary }}>1234 5678 9123 4567</TextFormatted>
                    <View style={{ flexDirection: 'row', marginTop: 10 }}>
                      <TextFormatted style={{ fontSize: 14, fontWeight: '400', color: theme.colors.primary, flex: 1 }}>Alexander Doe</TextFormatted>
                      <TextFormatted style={{ fontSize: 14, fontWeight: '400', color: theme.colors.primary }}>01/29</TextFormatted>
                    </View>
                  </View>
                  {index == 0 && (
                    <TouchableOpacity
                    ...
                    >
                      <Image source={require('../../assets/icons/edit_pen.png')} style={{ height: 24, width: 24, resizeMode: 'contain' }} />
                      <TextFormatted style={{ fontSize: 14, fontWeight: '600', color: theme.colors.primary, marginLeft: 5 }}>Edit card</TextFormatted>
                    </TouchableOpacity>
                  )}
                </ImageBackground>
                {show && (
                  <View style={{ position: 'absolute' }}>
                    ...
                    >
                      <Image source={require('../../assets/icons/trash.png')} style={{ height: 24, width: 24, resizeMode: 'contain' }} />
                      <TextFormatted style={{ fontSize: 14, fontWeight: '600', color: theme.colors.primaryBlack }}>Delete</TextFormatted>
                    </TouchableOpacity>
        ...
        <View style={{ height: 20 }} />
        <ButtonRow title={'Add new card'} onPress={() => refRBSheet2.current.open()} />
