  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      ...
      <ScrollView>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity
              onPress={() => navigation.navigate('viewPlan', { currentIndex: index })}
              style={{
                marginLeft: 20,
                width: 230,
                height: 270,
            ...
            >
              <View>
                <ImageBackground
                  ...
                  resizeMode="cover"
                >
                  <TextFormatted style={{ fontSize: 20, fontWeight: '700', color: theme.colors.primary, margin: 20 }}>{item.title}</TextFormatted>
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center' }}>
                    {item.data?.map(v => (
                      <View style={{ alignItems: 'center', marginTop: 10, marginHorizontal: 22 }}>
                        <Image source={v.img} style={{ height: 34, width: 34, resizeMode: 'contain' }} />
                        <TextFormatted style={{ fontSize: 16, fontWeight: '600', color: theme.colors.primary, marginTop: 10 }}>{v.no}</TextFormatted>
                      </View>
                    ))}
                  </View>
                </ImageBackground>
                <ImageBackground
                  ...
                  style={{ height: 80, width: 80, position: 'absolute', marginTop: -15, right: 15, alignItems: 'center' }}
                  resizeMode="contain"
                >
                  <TextFormatted
                    style={{
              ...
              {index > 0 && (
                ...
                >
                  <TextFormatted style={{ fontWeight: '900', fontSize: 30, color: theme.colors.primary }}>Coming soon...</TextFormatted>
                </View>
              )}
            </TouchableOpacity>
          )}
        ...
        <ButtonRow title={'Profile settings'} onPress={() => navigation.navigate('profileSetting')} />
        <ButtonRow title={'Purchase upgrades'} onPress={() => refRBSheet1.current.open()} />
        <ButtonRow title={'Appearance'} onPress={() => navigation.navigate('appearance')} />
        <ButtonRow title={'Security'} onPress={() => navigation.navigate('security')} />
        <ButtonRow title={'About Us'} onPress={() => navigation.navigate('aboutUs')} />
      ...
      <TouchableOpacity onPress={() => refRBSheet.current.open()}>
        ...
        >
          <TextFormatted style={{ fontSize: 18, fontWeight: '600', color: theme.colors.primary, flex: 1, marginRight: 10 }}>
            Refer a friend and get rewarded
          </TextFormatted>

          <Image source={Gift_img} style={{ height: 55, width: 55, resizeMode: 'contain' }} />
        </LinearGradient>
      </TouchableOpacity>
      <BottomSheet refRBSheet={refRBSheet} height={500}>
        ...
        >
          <View style={{ height: 4, width: 34, backgroundColor: '#fff', borderRadius: 10, marginTop: 10, alignSelf: 'center' }} />
          <Fontisto
            name="close-a"
            size={14}
            color="#fff"
            style={{ alignSelf: 'flex-end', marginRight: 20, marginVertical: 5, paddingHorizontal: 10 }}
          ...
          <ScrollView>
            <Image source={Gift_img} style={{ height: 148, width: 148, resizeMode: 'contain', alignSelf: 'center', marginTop: 20 }} />
            <TextFormatted
              style={{ fontSize: 18, fontWeight: '600', color: theme.colors.primary, textAlign: 'center', marginHorizontal: 30, marginTop: 30 }}
            >
              Get 1 month pro plan for free by inviting a friend to ...
            </TextFormatted>
            <TextFormatted
              style={{ fontSize: 14, fontWeight: '400', color: theme.colors.primary, textAlign: 'center', marginHorizontal: 30, marginTop: 30 }}
            >
              Share your unique link below to invite friends to ... Once they start using the app, you will get one month on the basic plan free.
            </TextFormatted>
          </ScrollView>
          <Button marginBottom={20} buttonName={'Copy referal link'} color={theme.colors.primary} onPress={() => refRBSheet.current.close()} />
        </LinearGradient>
      </BottomSheet>
...
const ButtonRow = ({ title, onPress }) => {
  ...
  return (
    ...
    >
      <TextFormatted
        style={{ fontSize: 15, fontWeight: '600', color: ThemeMode.selectedTheme ? theme.colors.primaryBlack : theme.colors.primary, flex: 1 }}
      >
        {title}
      </TextFormatted>
