...
const SuccessPassword = ({ navigation }) => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      <HeaderImage>
        <Header title={isCode == PASSCODE ? 'Passcode Recovery' : 'Password Recovery'} left marginTop={25} />
        <View style={{ height: 20 }} />
        <Logo />
      </HeaderImage>
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }}>
        <View style={{ marginHorizontal: 30, alignItems: 'center' }}>
          <Image source={require('../../assets/icons/checked.png')} style={{ height: 97, width: 97, resizeMode: 'contain', alignSelf: 'center' }} />
          <TextFormatted
            style={{
              fontSize: 18,
              fontWeight: '600',
          ...
          >
            Your {isCode == PASSCODE ? 'passcode' : 'password'} has been successfully reset
          </TextFormatted>
        </View>
      </ScrollView>
      <Button
