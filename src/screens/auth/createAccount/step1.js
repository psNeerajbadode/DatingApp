import { Image, ScrollView, StyleSheet, TextInput, TouchableOpacity, useWindowDimensions, View, Dimensions } from 'react-native';
...
const Step1 = ({ navigation }) => {
  ...
  async function Signup_step1() {
    try {
      ...
      body.append('user_id', Staps.User_id);
      body.append('image', {
        name: filerimg?.substring(filerimg?.lastIndexOf('/') + 1, filerimg?.length),
        type: uri.type,
        uri: filerimg,
      });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      <View style={{ zIndex: 0 }}>
        <HeaderImageShadow>
          <Pagination title={'Create account'} subTitle={'Personal detail'} position={1} />
          {uri == '' ? (
            <View
              style={{
                height: 135,
            ...
            >
              <TouchableOpacity onPress={() => refRBSheet3.current.open()}>
                <Image
                  ...
                  style={{ height: 65, width: 65, resizeMode: 'contain', marginLeft: 10 }}
                />
              </TouchableOpacity>
              <TextFormatted
                style={{
            ...
            <View style={{ height: 147, width: 147, alignSelf: 'center', marginTop: 20 }}>
              <Image
                resizeMode="contain"
                source={filerimg == '' ? { uri: uri.uri } : { uri: filerimg }}
                style={{ height: 147, width: 147, alignSelf: 'center', borderRadius: 40 }}
              />
          ...
          >
            {filterstate === 0 && (
              <ViewShot ref={Shotref} options={{ fileName: 'UserProfile', format: 'jpg', quality: 0.9 }}>
                  ...
                  <Contrast amount={contra}>
                    <Saturate amount={Satr}>
                      <Image
                        ...
                        style={{ height: dimension.width / 1.5, width: dimension.width / 1.5, alignSelf: 'center', borderRadius: 40, marginTop: 30 }}
                      />
                    </Saturate>
                  </Contrast>
                </Brightness>
                  ...
                  <Contrast amount={contra}>
                    <Saturate amount={Satr}>
                      <Image
                        resizeMode="cover"
                        source={cropImg == '' ? { uri: uri.uri } : { uri: cropImg }}
                        style={{ alignSelf: 'center', width: 240, height: 240, marginVertical: 20, borderRadius: 40 }}
                      />
                    </Saturate>
                  </Contrast>
                </Brightness>
                ...
                >
                  <Image
                    ...
                    style={{
                      ...
                      resizeMode: 'contain',
                      alignSelf: 'center',
                      tintColor: ThemeMode.selectedTheme ? theme.colors.darkGrey : theme.colors.primary,
                    }}
                  />
                  <TextFormatted
                    style={{
                      fontSize: 14,
                      fontWeight: '600',
                      color: ThemeMode.selectedTheme ? theme.colors.darkGrey : theme.colors.primary,
                      marginTop: 11,
                    }}
                  >
                    Brightness
            ...
            <View style={{ position: 'absolute', bottom: 0, width: '100%' }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', alignSelf: 'center', marginVertical: 20 }}>
                <TouchableOpacity
                ...
                >
                  <LinearGradient
                    ...
                    colors={ThemeMode.selectedTheme ? theme.colors.primaryOff : theme.colors.blackOn}
                  >
                    <Icon
                      name="ios-crop-sharp"
                      color={ThemeMode.selectedTheme ? theme.colors.darkGrey : theme.colors.primary}
                ...
                >
                  ...
                  >
                    <Image
                      ...
                      resizeMode="contain"
                      style={{ width: 22, height: 20, marginRight: 10, tintColor: theme.colors.primary }}
                    />
                    <TextFormatted
                      style={{
                        fontSize: 18,
      ...
      >
        ...
        <DropDown
          label={'Gender'}
          //labelColor={ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack}
      ...
      <View style={{ position: 'absolute', bottom: 0, width: '100%' }}>
        <ButtonView>
          <Bottom visible={visible} setVisible={setVisible} refRBSheet={refRBSheet} selectedDate={selectedDate} setSelectedDate={setSelectedDate} />

          <Button
            opacity={!uri || !name || !surname || !selectedDate || !gender || !showMe || !aboutMe ? 0.5 : 1}
            buttonName={'Next'}
            color={'#fff'}
            onPress={() => Signup_step1()}
            loading={Loading}
            marginBottom={1}
            disabled={!uri || !name || !surname || !selectedDate || !gender || !showMe || !aboutMe ? true : false}
            marginTop={1}
          />
    ...
    <View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 10 }}>
        <TouchableOpacity onPress={onPress} style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
          <Icon name="arrow-back-outline" color={ThemeMode.selectedTheme ? theme.colors.Black : theme.colors.primary} size={20} />
          <TextFormatted
            style={{
              fontSize: 14,
              fontWeight: '600',
