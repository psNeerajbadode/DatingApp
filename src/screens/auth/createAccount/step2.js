  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      <HeaderImage_1 height={170} marginBottom={1}>
        <Pagination title={'Create account'} subTitle={'Insights'} position={2} />
      </HeaderImage_1>
      <ScrollView>
        <DropDown
          label={'Sexual orientation'}
          placeholder={'Select your sexual orientation'}
          items={['Staight', 'Gay', 'Lesbian', 'Bisexual', 'Asexual', 'Demisexual', 'Pansexual', 'Queer', 'Questioning']}
        ...
        <DropDown
          ...
          items={['Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpius', 'Sagittarius', 'Capricornus', 'Aquarius', 'Pisces']}
          onSelect={(selectedItem, index) => {
        ...
        <SwitchBox Name={'Do you smoke?'} onPress={() => setToggle1(!toggle1)} toggle={toggle1} />
        <SwitchBox Name={'Do you have kids?'} onPress={() => setToggle2(!toggle2)} toggle={toggle2} />
        <SwitchBox Name={'Do you drink?'} onPress={() => setToggle3(!toggle3)} toggle={toggle3} />
      </ScrollView>
      <View
        style={{
          height: 90,
      ...
      >
        <Button buttonName={'Next'} marginBottom={15} Loading={Loading} marginTop={15} onPress={() => Signup_step2()} />
      </View>
    </View>
  );
};
