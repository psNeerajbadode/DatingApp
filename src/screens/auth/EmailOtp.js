...
const EmailOtp = ({ navigation }) => {
  ...
  const [state, setState] = useState({ pin1: '', pin2: '', pin3: '', pin4: '', pin5: '' });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ThemeMode.selectedTheme ? theme.colors.primary : theme.colors.primaryBlack }}>
      <HeaderImage>
        <Header
          title={params?.isSignup ? 'Sign up' : isPasscode ? 'Passcode Recovery' : 'Password Recovery'}
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'space-between' }}>
        <View style={{ marginVertical: 20 }}>
          ...
          >
            Please check your inbox and enter the {isPasscode ? 'change passcode' : 'received code below'}
          </TextFormatted>
            ...
            <TextInput
              style={[
                styles.textInput,
                {
                  ...
                  color: ThemeMode.selectedTheme ? theme.colors.primaryprimaryBlack : theme.colors.primary,
                },
              ]}
              ref={textInputRef1}
              value={state.pin1}
          ...
          <Button
            opacity={state.pin1 && state.pin2 && state.pin3 && state.pin4 && state.pin5 ? 1 : 0.5}
            onPress={() => Email_Otp()}
            Loading={Loading}
            buttonName={'Verify'}
            color={theme.colors.primary}
            containerStyle={{ marginTop: 20 }}
            disabled={state.pin1 && state.pin2 && state.pin3 && state.pin4 && state.pin5 ? false : true}
          />
        </View>
